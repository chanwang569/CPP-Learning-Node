...
string对象通常比char* 更好一点
对于class的专属常量，为了限制作用域在class内,并且防止产生多个实体，最好使用static
如果你的编译器支持在类内对const static 整数类型声明时获初值，则使用
如果不支持，则在类内定义，在对应的实现文件中赋值
规则
...
RAII 处理拷贝行为可选择：
  1. 当我们处理不能同步拥有的资源的时候，可以采用禁止复制，如把copying操作声明为private  
...
在先执行new Widget`语句和调用std::tr1::shared_ptr构造函数之间
不能确定priority函数的执行顺序，可能在最前面，也可能在他们的中间
四、设计与声明
...
合理的构建class的构造函数、析构函数和内存分配函数以及释放函数
不能把初始化和赋值搞混了
如果你的类需要被用来以值传递，复制构造函数应该设计一个通过值传递的版本
你应该给你的成员变量加约束条件，保证他们是合法值，所以你的成员函数必须担负起错误检查工作
如果你是派生类，那么你应该遵守基类的一些规范，如析构函数是否为virtural
你是否允许你的class有转换函数，，是否允许隐式转换。如果你只允许explicit构造函数存在，就得写出专门负责执行转换的函数
想清楚你的类应该有哪些函数和成员
哪些应该设计为私有
哪个应该是你的friend,以及将他们嵌套与另一个是否合理
对效率，异常安全性以及资源运用提供了哪些保证
如果你定义的不是一个新type,而是定义整个type家族，那么你应该定义一个类模板
如果只是定义新的子类以便为已有的类添加机制，说不定单纯定义一个或多个non-member
？条款20：宁以pass-by-reference-to-const替换pass-by-value
...
在类中提供一个 public swap成员函数，并且这个函数不能抛出异常
在类的命名空间中提供一个non-member swap函数，并令它调用类中的swap函数
如果你正在编写一个类而不是模板类，为你的class特化std::swap函数，并令它调用你的swap函数
在类中声明 using std::swap,让其暴露，使得编译器自行选择更合适的版本
五、实现
...
const_cast 通常被用来将对象的常量性转除。它也是唯一有此能力的转型操作符
dynamic_cast 主要用来执行“安全向下转型” ，也就是用来决定对某对象是否归属继承体系中的某个类型。它是唯一无法由旧式语法执行的动作，也是唯一可能耗费重大运行成本的转型动作
...
如果可以的话，避免dynamic_cast转型，如果实在需要，则可以试着用别的无转型方案代替
如果转型是必要的，那么应该把他隐藏于某个函数背后，客户随后可以调用该函数，而不是需要将转型放进自己的代码里
宁可要新型转型，也不要使用旧式转型
条款28：避免返回handles指向对象内部成分
...
首先以对象管理资源可以阻止资源泄漏
在你能实现的情况下，尽量满足以上的最高等级
条款30：透彻了解inlining 的里里外外

inline 声明的两种方式:

隐喻的inline申请，即把定义写在class内部
明确声明，即在定义式前加上关键字inline
...
在基类函数调用之前加上 this->
使用 using 声明式 ,告诉编译器，请它假设这个函数存在
指出这个函数在基类中，使用基类::函数的形式写出来（不推荐这个，因为如果是virtual函数，则 会影响动态绑定）
...
让更多内存可被使用。此策略的一个做法是，程序一开始就分配一大块内存，而后当其第一次被调用，将它释还给程序使用
安装另一个new—handler。可以设置让其调用另一个new—handler来替换自己，用来做不同的事情，其做法是调用set_new_handler
卸载new—handler,也就是将null指针传给set_new_handler，这样new在分配不成功时抛出异常
抛出bad_alloc的异常。
不返回，调用abort或exit
C++并部支持类的专属new—handler，但其实也不需要。你可以令每个类提供自己的set_new_handler和operator new即可 set_new_handler允许客户指定一个函数，在内存分配无法获得满足时调用。
